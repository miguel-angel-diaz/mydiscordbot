...
class SwissManager:
   ...
   # utils/swiss_manager.py
    ...
    async def crear_torneo(self, codigo: str, nombre: str, max_jugadores: int, nivel: str, fecha_inicio: str = None):
        """Crea los canales y registra el torneo en torneos-activos con formato bonito."""
        ...
        # Registrar en torneos-activos con formato IDÉNTICO a Challonge
        canal_activos = await self._get_channel("torneos-activos")
        if canal_activos:
            mensaje = (
            ...
            await canal_activos.send(mensaje)

        # Guardar en el estado (para la web)
        from utils.torneos_estado import actualizar_torneo_estado
        await actualizar_torneo_estado(self.bot, codigo, {
            "nombre": nombre,
            "nivel": nivel,
        ...
        # Estado interno (ronda actual y byes)
    ...
    async def generar_ronda(self, codigo: str, ronda_num: int):
        ...
        for i, (jugador, pts) in enumerate(ordenados):
            ...
            oponente = self._buscar_oponente(jugador, ordenados, usados, historial, pts, puntuaciones)
            if oponente:
    ...
    def _buscar_oponente(self, jugador, ordenados, usados, historial, pts, puntuaciones):
